  ...
  return (
    <div style={{ padding: '0 0 40px' }}>
      ...
      <div style={{ padding: '0 32px' }}>
        ...
        {/* Portfolio rendement overzicht */}
        {beleggingen.length > 0 && (() => {
          const totaalKostprijs = gefilterdeBeleggingen.reduce((sum, b) => {
            const factor = getMuntFactor ? getMuntFactor(b.munt || 'EUR') : ((b.munt || 'EUR') === 'USD' ? 0.865 : 1);
            return sum + b.kostprijs * b.aantal * factor;
          }, 0);
          const totaalHuidigeWaarde = gefilterdeBeleggingen.reduce((sum, b) => {
            const koers = koersen[b.symbol];
            const huidigePrijs = koers ? koers.c : b.kostprijs;
            const factor = getMuntFactor ? getMuntFactor(b.munt || 'EUR') : ((b.munt || 'EUR') === 'USD' ? 0.865 : 1);
            return sum + huidigePrijs * b.aantal * factor;
          }, 0);
          const totaalWinst = totaalHuidigeWaarde - totaalKostprijs;
          const totaalWinstPct = totaalKostprijs > 0 ? (totaalWinst / totaalKostprijs) * 100 : 0;
          const maxWaarde = Math.max(totaalKostprijs, totaalHuidigeWaarde);

          return (
            <div className="card" style={{ marginTop: 16 }}>
              <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 4 }}>Rendement</div>
              <div style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 20 }}>Totale prestatie van je portfolio</div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 20 }}>
                ...
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: 'var(--text-muted)', marginBottom: 4 }}>
                    <span>Kostprijs</span>
                    <span>€{totaalKostprijs.toLocaleString('nl-BE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                  </div>
                  <div style={{ height: 12, background: 'var(--bg)', borderRadius: 6, overflow: 'hidden' }}>
                    <div style={{
                      height: '100%', borderRadius: 6,
                      width: `${(totaalKostprijs / maxWaarde) * 100}%`,
                      background: 'var(--text-muted)'
                    }} />
                  </div>
                </div>

                {/* Huidige waarde staaf */}
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: 'var(--text-muted)', marginBottom: 4 }}>
                    <span>Huidige waarde</span>
                    <span>€{totaalHuidigeWaarde.toLocaleString('nl-BE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                  </div>
                  <div style={{ height: 12, background: 'var(--bg)', borderRadius: 6, overflow: 'hidden' }}>
                    <div style={{
                      height: '100%', borderRadius: 6,
                      width: `${(totaalHuidigeWaarde / maxWaarde) * 100}%`,
                      background: totaalWinst >= 0 ? 'var(--green)' : 'var(--red)',
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', background: totaalWinst >= 0 ? 'var(--green-bg)' : 'var(--red-bg)', borderRadius: 10 }}>
                <div style={{ fontSize: 14, fontWeight: 600, color: totaalWinst >= 0 ? 'var(--green)' : 'var(--red)' }}>
                  Totale winst/verlies
                </div>
                <div style={{ textAlign: 'right' }}>
                  <div style={{ fontSize: 18, fontWeight: 700, color: totaalWinst >= 0 ? 'var(--green)' : 'var(--red)' }}>
                    {totaalWinst >= 0 ? '+' : ''}€{Math.abs(totaalWinst).toLocaleString('nl-BE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </div>
                  <div style={{ fontSize: 13, color: totaalWinst >= 0 ? 'var(--green)' : 'var(--red)' }}>
                    {totaalWinstPct >= 0 ? '+' : ''}{totaalWinstPct.toFixed(2)}%
                  </div>
                </div>
              </div>
            </div>
