...
// Haalt beleggingen op uit Vercel Blob, live koersen via Finnhub, stuurt mooie mail
...
async function haalKoersOp(symbol, munt) {
  try {
    // Probeer eerst Yahoo Finance (werkt voor Europese aandelen)
    const yfSym = symbol;
  ...
  try {
    ...
    if (d.c) return { c: d.c, pc: d.pc || d.c };
  } catch (e) {}
  return null;
}
...
function genereerEmailHTML({ gebruiker, beleggingen, koersen, totaalWaarde, dagWinst, dagPct, datum }) {
  ...
  const beleggingRijen = beleggingen.map(b => {
    ...
    const isP = dagVPct >= 0;
    const logo = b.logo ? `<img src="${b.logo}" width="32" height="32" style="border-radius:8px; object-fit:contain; border:1px solid #e2e8f0; background:white; padding:2px; vertical-align:middle;" />` : `<div style="width:32px;height:32px;border-radius:8px;background:#6366f1;display:inline-flex;align-items:center;justify-content:center;color:white;font-size:11px;font-weight:700;vertical-align:middle;">${b.symbol.slice(0,2)}</div>`;
    return `
      <tr style="border-bottom: 1px solid #f1f5f9;">
        ...
        <td style="padding: 12px 0; text-align: right; vertical-align: middle;">
          ...
          <div style="color: ${isP ? '#16a34a' : '#dc2626'}; font-size: 12px; font-weight:500;">
            ${isP ? '+' : ''}${dagVPct.toFixed(2)}%
          </div>
        </td>
      </tr>`;
  }).join('');
...
export default async function handler(req, res) {
  ...
  if (!MATICO_EMAIL || !RESEND_API_KEY || !FINNHUB_KEY) {
    return res.status(500).json({ error: 'Ontbrekende environment variables' });
  }
  ...
  try {
    ...
    return res.status(500).json({ error: 'Geen beleggingen gevonden in Blob — sync eerst via Instellingen' });
  }

  // Live koersen ophalen voor elke belegging
  const koersen = {};
  ...
  // Totale waarde en dagwinst berekenen
  let totaalWaarde = 0, dagWinst = 0;
  beleggingen.forEach(b => {
    const k = koersen[b.symbol];
    const factor = (b.munt || 'EUR') === 'USD' ? 0.92 : 1;
    const waarde = k ? k.c * b.aantal * factor : b.kostprijs * b.aantal;
    const dv = k ? (k.c - k.pc) * b.aantal * factor : 0;
    totaalWaarde += waarde;
    dagWinst += dv;
  });
  const dagPct = totaalWaarde > 0 ? (dagWinst / (totaalWaarde - dagWinst)) * 100 : 0;

  const datum = new Date().toISOString();
  const html = genereerEmailHTML({ gebruiker, beleggingen, koersen, totaalWaarde, dagWinst, dagPct, datum });
